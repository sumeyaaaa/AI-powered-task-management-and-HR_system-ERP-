...
def show_classified_task_card(task, index, task_manager):
    ...
    with st.container():
        ...
        with col1:
            ...
            with col1a:
                # ========== ADDED: CREATED DATE AND ASSIGNED DATE ==========
                ...
                # NEW: Show assigned date if available
                assigned_at = task.get('assigned_at', '')
                if assigned_at:
                    try:
                        assigned_date = datetime.fromisoformat(assigned_at.replace('Z', '')).date()
                        st.write(f"**Assigned:** {assigned_date}")
                    except:
                        st.write(f"**Assigned:** {assigned_at[:10]}")
...
def show_admin_task_details_tab(task, task_manager):
    ...
    with col1:
        ...
        # UPDATED: Add Created Date and Assigned Date
        created_at = task.get('created_at', '')
        if created_at:
            try:
                created_date = datetime.fromisoformat(created_at.replace('Z', '')).date()
        ...
        # FIXED: Correct field name and label
        assigned_at = task.get('assigned_at', '')  # Fixed: 'assigned_a' -> 'assigned_at'
        if assigned_at:
            try:
                assigned_date = datetime.fromisoformat(assigned_at.replace('Z', '')).date()
                st.write(f"**Assigned:** {assigned_date}")  # Fixed: "Created" -> "Assigned"
            except:
                st.write(f"**Assigned:** {assigned_at[:10]}")  # Fixed: "Created" -> "Assigned"
        
        # Safe due date access
        due_date = task.get('due_date', 'Not set')
        if due_date and due_date != 'Not set':
            st.write(f"**Deadline:** {due_date[:10]}")
        else:
...
def show_employee_task_details_tab(task, task_manager):
    ...
    with col1:
        ...
        # Show objective information - UPDATED: Remove pre-number and ID
        objectives_data = task.get('objectives')
        ...
        # UPDATED: Add Created Date before Due Date
        created_at = task.get('created_at', '')
        if created_at:
            try:
                created_date = datetime.fromisoformat(created_at.replace('Z', '')).date()
